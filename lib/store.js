...
// Count how many of the 12 possible bingo lines (5 rows + 5 columns + 2
// diagonals) are completed on a 25-cell board, given an array of booleans
// where `true` means the cell is filled. Returns 0 if the board shape is
// invalid. A fully-filled board (all 25 cells true) returns 12.
...
// Map a user's email domain to one of the four ETH-domain research institutes
// (or "External" for everyone else). Used to badge winners on the public
// Winners page. Anonymous winners do NOT receive an institute badge — see the
// privacy carve-out in getWinners() below.
function instituteForEmail(email) {
  if (!email || typeof email !== 'string') return null;
  const lower = email.toLowerCase();
  if (lower.endsWith('@psi.ch'))   return { code: 'psi',      label: 'PSI',      color: '#00017F' };
  if (lower.endsWith('@empa.ch'))  return { code: 'empa',     label: 'Empa',     color: '#BE0000' };
  if (lower.endsWith('@eawag.ch')) return { code: 'eawag',    label: 'Eawag',    color: '#118EC6' };
  if (lower.endsWith('@wsl.ch'))   return { code: 'wsl',      label: 'WSL',      color: '#006268' };
  return { code: 'external', label: 'External', color: null };
}
...
class Store {
  ...
  getWinners() {
    ...
    return this._data.winners
      ...
      .map(w => {
        ...
        // Compute bingo metrics from the ORIGINAL snapshot (with URLs) so
        // that anonymous winners get the same count as everyone else even
        // though their snapshot is stripped below.
        const bools = (w.snapshot || []).map(c => !!(c && c.url));
        const bingoCount = countBingos(bools);
        const filledCount = bools.filter(b => b).length;
        const isSuperBingo = filledCount === 25;

        if (isAnonymous) {
          // Anonymous winners: strip evidence URLs and descriptions, AND do
          // not attach an institute label. The latter would publish a derived
          // attribute of the user's email, which the Privacy Notice says we
          // do not do for anonymous accounts.
          ...
          return Object.assign({}, w, { snapshot, anonymous: true, bingoCount, isSuperBingo });
        }
        // Non-anonymous winners: keep the snapshot intact and add a derived
        // institute label based on the user's email domain.
        const institute = user ? instituteForEmail(user.email) : null;
        return Object.assign({}, w, { institute, bingoCount, isSuperBingo });
      });
  }
  ...
  // Clear a user's wonAt timestamp. Used when a user edits their board such
  // that they no longer have a bingo — they should no longer appear as a
  // winner. The wonAt field is set fresh whenever they complete (and submit)
  // a new bingo.
  clearUserWonAt(userId) {
    const user = this._data.users.find(u => u.id === userId);
    if (user) user.wonAt = null;
  }

  // Remove a user's entry from the winners list without touching the user
  // record itself. Returns true if an entry was removed.
  removeWinnerByUserId(userId) {
    const before = this._data.winners.length;
    this._data.winners = this._data.winners.filter(w => w.userId !== userId);
    return this._data.winners.length < before;
  }

  // True if the given userId currently has a winner entry on the board.
  hasWinnerEntry(userId) {
    return this._data.winners.some(w => w.userId === userId);
  }

  // Refresh an existing winner entry's snapshot so the Winners page shows
  // the user's CURRENT board state (and current bingo count). Called on
  // every board update for a user who is already a winner — e.g. when they
  // extend a 1-bingo board into a 2-bingo board, the count needs to update
  // even though the auto-submit only fires on the no-bingo->bingo transition.
  updateWinnerSnapshot(userId, board) {
    const w = this._data.winners.find(x => x.userId === userId);
    if (w) w.snapshot = board;
  }
}

module.exports = Store;
// Re-export the bingo-counting helper so server.js can use the same logic
// for /api/bingo validation and for board-update refresh.
module.exports.countBingos = countBingos;
